...
import { Profile, UserRole, BRANCH_LIST, MEGABRANCH_LIST, ALL_SYSTEM_BRANCHES, isRegionalHeadRole, WEST_BRANCHES, CENTRAL_BRANCHES, EAST_BRANCHES, hasRole } from '../types';
...
export default function UserManagement({ 
  ...
  const [role, setRole] = useState<string>('ASO');
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    onAddUser(email.trim().toLowerCase(), fullName.trim(), role as UserRole, branch, password.trim() || undefined);
  ...
  const openEditForm = (p: Profile) => {
    ...
    setBranchSearch('');
    setBranchTab(hasRole(p.role, 'ASO Megabranch') ? 'megabranch' : 'all');
    setShowForm('edit');
  };
